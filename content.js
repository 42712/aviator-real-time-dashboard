...
(function () {
  ...
  function processarMensagemWS(txt) {
    try {
      // Remove prefixos de protocolo (ex: "42[...]" do socket.io, "0{...}" etc)
      ...
      // Navega na estrutura para encontrar o multiplicador
      const mult = extrairMultiplicadorJSON(data);
      if (mult !== null && isFinite(mult) && mult >= 1) {
        // Detecta se rodada está ativa (mult > 1 e crescendo)
        const crescendo = lastMultiplier !== null && mult > lastMultiplier;

        if (mult < 1.02 && lastMultiplier !== null && lastMultiplier > 1.1) {
          // Rodada reiniciou — envia round_start
          sendRoundStart(lastRound);
          lastMultiplier = null;
        }

        if (crescendo || mult >= 1.0) {
          sendMultRT(mult);
          lastMultiplier = mult;
        }
      }

      // Detecta crash/fim de rodada na mensagem WS
      const crashed = detectarCrashJSON(data);
      if (crashed && crashed.mult) {
        const cor = detectarCorJSON(data);
        sendCandle(crashed.mult, cor, crashed.round || lastRound);
        lastMultiplier = null;
      }
