...
(function () {
  ...
  const INTERVAL_MS = 200; // poll a cada 200ms para multiplicador suave

  let lastMultiplier  = null;
  let lastRound       = null;
  let prevCrashed     = false;
  let sending         = false;
  let sendingMult     = false;
  ...
  async function sendMultRT(mult) {
    if (sendingMult) return;
    if (lastMultSent !== null && Math.abs(mult - lastMultSent) < 0.01) return;
    sendingMult = true;
    lastMultSent = mult;
    try {
      await fetch(`${SERVER}/api/mult`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mult, ts: Date.now() }),
      });
    } catch (_) {
      // silencia erros do mult RT para não logar spam
    } finally {
      sendingMult = false;
    }
  }
